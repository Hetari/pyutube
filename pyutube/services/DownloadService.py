...
from . import VideoService, AudioService, DownloadService, FileService
...
class DownloadService:
    def __init__(self, url: str, path: str, quality: str = '720p', is_audio: bool = False):
        ...
        self.video_service = VideoService(self.url)
        self.audio_service = AudioService(self.url)
        self.file_service = FileService(self.path, self.is_audio)
    ...
    def download(self) -> bool:
        ...
        if self.is_audio:
            self.download_audio(video_audio, video_id, video_file)
        else:
            self.download_video(video, video_id, video_file, video_audio)

        return True

    def download_audio(self, video_audio: YouTube, video_id: str, video_filename: str) -> bool:
